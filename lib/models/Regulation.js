...
module.exports = class Regulation {
  ...
  static async getFishRegDetail(id) {
    const { rows } = await pool.query(`
    SELECT fish.*,
    COALESCE (json_agg(to_jsonb(regulations))
    FILTER (WHERE regulations.id IS NOT NULL), '[]')
    AS total_info
    FROM fish
    LEFT JOIN fish_regulations ON fish_regulations.fish_id = fish.id
    LEFT JOIN fish ON regulations.id = fish_regulations.regulations_id
    WHERE fish.id = $1
    GROUP BY fish.id
    `, [id]);
    if (!rows[0]) return null;
